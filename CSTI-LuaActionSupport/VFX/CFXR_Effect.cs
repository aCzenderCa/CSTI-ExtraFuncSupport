...
namespace CartoonFX
{
	...
	public partial class CFXR_Effect : MonoBehaviour
	{
		...
		[Space]
		public AnimatedLight[] animatedLights = Array.Empty<AnimatedLight>();
		...
		[NonSerialized] MaterialPropertyBlock materialPropertyBlock;
		[NonSerialized] Renderer particleRenderer;
		...
		public void FadeOut(float fo_time)
		{
			if (!isFadingOut)
			{
				isFadingOut = true;
				fadingOutStartTime = fo_time;
			}

			foreach (var animLight in animatedLights)
			{
				animLight.animateFadeOut(fo_time - fadingOutStartTime);
			}
		}
#endif
